...
router.post("/fileupload/:id", function (req, res) {
  ...
  function checkAuthority(a, b) {
    conn.query(
      `insert into mdl_group_file(userid, filepath ) 
        values(${a},"${b}")`
    );
  }
  ...
  form.parse(req, function (err, fields, files) {
    if (files.filetoupload.name.split(".").length > 2) {
      res.send(`<script type=bodypart.js"text/javascript">
    ...
    for (var i in filelist) {
      if (filelist[i] == files.filetoupload.name) p++;
      var tempname = sname + "_" + p + "." + ext;
      if (filelist[i] == tempname) p++;
    }
    ...
    var newpath;
    if (p > 1) newpath = fpath + "/" + sname;
    else newpath = fpath + "/" + files.filetoupload.name;

    fs.rename(oldpath, newpath, function (err) {
      checkAuthority(req.session.userid, newpath);
...
router.post("/delete/:id", function (req, res) {
  ...
  var leader;
  var dbCourseId = req.session.course[req.session.courseId].id;
  var loginId = req.session.userid;
  var authority;

  function checkleader(uid, cid, callback) {
    conn.query(
      `SELECT roleid
            from mdl_role_assignments
            where userid = ${uid} and contextid = 
            (select distinct id from mdl_context
                where contextlevel=50 and instanceid=${cid})`,
      (error, rows) => {
        leader = rows[0].roleid;
        callback();
      }
    );
  }

  function deletepath(b) {
    conn.query(
      `delete from mdl_group_file
            where filepath = "${b}"`
    );
  }

  function checkAuthority(b, callback) {
    conn.query(
      `SELECT userid
            from mdl_group_file
            where filepath="${b}"`,
      (error, rows) => {
        authority = rows[0].userid;
        callback();
      }
    );
  }

  checkAuthority(fpath, () => {
    checkleader(loginId, dbCourseId, () => {
      if (authority == loginId || leader == 9) {
