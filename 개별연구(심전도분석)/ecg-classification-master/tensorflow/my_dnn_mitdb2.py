...
def main():
    ...
    train_data, train_labels, eval_data, eval_labels = load_data(output_path, window_size, compute_RR_interval_feature,
                                                                 compute_wavelets, binary_problem)
    ...
    for c in range(0, num_classes):
        if count[c] > 0:
            ...
            class_weight[c] = float(max_class) / float(
                count[c])  # the class with more instance will have weight = 1, and the others x times ...
        ...
        for h2 in hn_2:
            for h3 in hn_3:
                for s in steps:
                    ...
                    np.savetxt('FC_' + str(h1) + '_' + str(h2) + '_' + str(h3) + '_' + str(s) + '_cm.txt',
                               confusion_matrix, fmt='%-7.0f')
                    np.savetxt('FC_' + str(h1) + '_' + str(h2) + '_' + str(h3) + '_' + str(s) + '_acc.txt', acc,
                               fmt='%-7.2f')
                    print(acc_g)
