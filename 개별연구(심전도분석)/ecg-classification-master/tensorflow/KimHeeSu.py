...
def my_model_fn(features, targets, mode, params):
  ...
  #print(type(int(features.shape[0])), features.shape[1])
  a = int(features.shape[0])
  b = int(features.shape[1])
  features = np.arange(a*b).reshape(a,b,1)
  features = tf.cast(features, dtype=tf.float32)
  img = tf.layers.conv1d(features, filters=16,kernel_size=5,padding="SAME",activation="relu")
  img = tf.layers.conv1d(img, filters=16, kernel_size=5, padding="SAME", activation="relu")
  img = tf.layers.max_pooling1d(img, pool_size=2,strides=2,padding="SAME")
  img = tf.layers.dropout(img, rate=0.2)

  img = tf.layers.conv1d(img, filters=32, kernel_size=3, padding="SAME", activation="relu")
  img = tf.layers.conv1d(img, filters=32, kernel_size=3, padding="SAME", activation="relu")
  img = tf.layers.max_pooling1d(img, pool_size=2, strides=2, padding="SAME")
  img = tf.layers.dropout(img, rate=0.2)

  img = tf.layers.conv1d(img, filters=64, kernel_size=3,padding="SAME",activation="relu")
  img = tf.layers.conv1d(img, filters=64, kernel_size=3, padding="SAME", activation="relu")
  img = tf.layers.max_pooling1d(img,pool_size=2,strides=2,padding="SAME")
  img = tf.layers.dropout(img,rate=0.2)
  #
  # img = tf.layers.conv1d(img, filters=256, kernel_size=3, padding="SAME", activation="relu")
  # img = tf.layers.conv1d(img, filters=256, kernel_size=3, padding="SAME", activation="relu")
  # img = tf.layers.max_pooling1d(img, pool_size=2, strides=2, padding="SAME")
  # img = tf.layers.dropout(img, rate=0.2)

  img = tf.layers.flatten(img)
  dense1 = tf.layers.dense(img,units=64,activation="relu")
  dense1 = tf.layers.dense(dense1, units=64,activation="relu")
  output_layer = tf.layers.dense(dense1, units=params["num_classes"],activation="softmax")
  # tf.nn.conv1d
...
def main():
  ...
  dataset = 'D:/download/ecg-classification-master/'
    ...
    for h2 in hn_2:
      for h3 in hn_3:
        for s in steps:
          ...
          np.savetxt('convolution_' + str(h1) + '_' + str(h2) + '_' + str(h3) + '_' + str(s) + '_cm.txt',
                     confusion_matrix, fmt='%-7.0f')
          np.savetxt('convolution_' + str(h1) + '_' + str(h2) + '_' + str(h3) + '_' + str(s) + '_acc.txt', acc,
                     fmt='%-7.2f')
          print(acc_g)
